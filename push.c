 ...
 * @stack: a double pointer to header (top) of the stack
...
void _push(stack_t **, unsigned int line_number)
{
	...
	len = strlen(argument);
	for (a = 0; a < len; a++)
		if (!isdigit(argument[a]) && argument[0] != '-')
		{
			dprint(STDERR_FILENO, "L%u: usage: push integer\n", line_number);
			free_stack_t(*stack);
	...
	if (stack_queue == 'q')
		add_note_queue(stack, atoi(argument));
}
